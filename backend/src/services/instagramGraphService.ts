...
function mapGraphError(err: any): string {
  ...
  if (data) {
    ...
    if (code === 190) {
      if (sub === 463) return 'Sessão expirada — o access_token expirou. Gere um novo token de longa duração e cole em Configurações > Contas.';
      if (sub === 467) return 'Sessão inválida — access_token inválido. Verifique se copiou o token completo.';
      return `Token inválido/expirado (190): ${msg}. Gere um novo token em developers.facebook.com > Graph API Explorer.`;
    }
    if (code === 10) return `Permissão negada (10): ${msg}. Verifique se seu App tem instagram_content_publish e se o Instagram está conectado à Página.`;
    if (code === 200) return `Permissão negada (200): ${msg}. Confirme que o Instagram é Profissional/Business e vinculado à Página.`;
    if (code === 4) return `Limite da API atingido (4): ${msg}. Aguarde alguns minutos.`;
    if (code === 100 && msg.includes('image')) return `Imagem inválida (100): ${msg}. Use JPG/PNG público (https) e <8MB.`;
    if (code === 368) return `Ação bloqueada (368): ${msg}. Conta em revisão temporária.`;
    return `Erro Graph API (${code}${sub ? '/' + sub : ''}): ${msg}`;
  }
  if (err?.code === 'ENOTFOUND' || err?.code === 'ETIMEDOUT') return 'Falha de rede ao contatar graph.facebook.com — verifique internet/proxy.';
...
export class InstagramGraphService {
   ...
   */
  static async publishWithToken(account: any, req: InstagramPublishRequest): Promise<InstagramPublishResult> {
    const token: string = String(account.access_token || account.accessToken || '').trim();
    const igUserId: string = String(account.ig_user_id || account.igUserId || '').trim();

    if (!token) return { success: false, status: 'FAILED', error: 'Conta sem access_token — cole o token de longa duração em Configurações > Contas (Instagram oficial).' };
    if (!igUserId) return { success: false, status: 'FAILED', error: 'Conta sem IG User ID — pegue em Graph API Explorer: GET /me/accounts -> instagram_business_account.id' };
    ...
    try {
      ...
      const createRes = await axios.post(createUrl, null, { params: createParams, timeout: 20000 });
   ...
   */
  static async validateCredentials(accessToken: string, igUserId: string): Promise<{ valid: boolean; error?: string; info?: any }> {
    try {
