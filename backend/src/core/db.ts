...
function loadStore() {
  try {
    if (fs.existsSync(DB_FILE)) {
      ...
      if (!Array.isArray((store as any).library_folders)) (store as any).library_folders = [];
    } else {
      saveStore();
    }
  } catch (err) {
    ...
    return {
      ...
      run: (...params: any[]) => {
        ...
        if (q.includes('INSERT INTO campaigns')) {
          ...
          let config: any = {};
          try { config = typeof configJson === 'string' ? JSON.parse(configJson) : (configJson || {}); } catch {}
          if (!store.library_folders) store.library_folders = [];
