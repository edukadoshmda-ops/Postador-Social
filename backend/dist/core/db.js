...
const dataDir = isVercel ? path_1.default.join(os_1.default.tmpdir(), 'pulso_data') : path_1.default.resolve(__dirname, '../../data');
const DB_FILE = path_1.default.join(dataDir, 'db.json');
...
function saveStore() {
    try {
        fs_1.default.writeFileSync(DB_FILE, JSON.stringify(store, null, 2), 'utf-8');
    }
    catch (err) {
        ...
        return {
            ...
            get: (...params) => {
                ...
                if (q.includes('FROM campaign_items') && q.includes('SELECT count(*) as total')) {
                    ...
                    if (!cid)
                        return { total: 0, completed: 0, successful: 0, pending: 0, failed: 0 };
            ...
            run: (...params) => {
                ...
                if (q.includes('INSERT INTO campaigns')) {
                    ...
                    if (c && statusLit) {
                        c.status = statusLit;
                        if (q.includes('started_at') && !c.started_at)
                            c.started_at = new Date().toISOString();
                        if (q.includes('finished_at'))
                            c.finished_at = new Date().toISOString();
                        if (statusLit === 'COMPLETED')
                    ...
                    if (acc)
                        acc.custom_limits = customLimits;
                }
